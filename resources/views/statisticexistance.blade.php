...
<body class="h-100">
    <section class="content py-4 mt-4 px-2">
            ...
            <div class="col-lg-3 col-12 mt-1 ">
                <div class="card bg-primary-01 px-3 py-4 border-0 shadow-none ">
                    ...
                    <div class="ml-2 my-3 offset-2 text-center">
                        <div class="my-2">
                            <h1 class="fas fas fa-user-check text-info"></h1>
                        </div>
                        <p class="">Statistik</p>
                        <h5><b>{{ $title }}</b></h5>
                    </div>

                    <form id="form_statistictarget" method="POST" action="{{ route('exportexistance.realize') }}"
                        ...
                        <table
                            ...
                            <thead>
                                <tr class="table-light">
                                    ...
                                    <th>Kode</th>
                                    <th>Provinsi</th>
                                    <th>Pelaksanaan Mekop</th>
                                    <th>Pasangan Usia Subur (PUS)</th>
                                    <th>Peserta KB Aktif (PA)</th>
                                    <th>Peserta KB Baru (PB)</th>
                                    <th>Putus Pakai (DO)</th>
                                    <th>Tidak Terlayani (UN)</th>
...
function openTblProvince() {
    ...
    mekop_province_table = $('.mekop_province_table').dataTable({
        ...
        "ajax": {
            "url": "{{ route('getallstatexistanceprov.existance') }}",
            "type": "POST",
            "headers": {
...
function openTblRegency(id, name) {
    ...
    if (!$('.breadcrumb').find('.breadcrumb-item').hasClass('bcregency')) {
        ...
        $('.breadcrumb').append(
            '<li class="bcregency breadcrumb-item active"  onclick="openTblRegency(' + id + ' , ' + "'" + name +
            "'" +
    ...
    mekop_regency_table = $('.mekop_regency_table').dataTable({
        ...
        "ajax": {
            "url": "{{ route('getallstatexistanceregency.existance') }}",
            "type": "POST",
            "headers": {
                "X-CSRF-TOKEN": "{{ csrf_token() }}",
            },
...
function openTblDistrict(id, name) {
    ...
    mekop_district_table = $('.mekop_district_table').dataTable({
        ...
        "ajax": {
            "url": "{{ route('getallstatexistancedistrict.existance') }}",
            "type": "POST",
            "headers": {
                "X-CSRF-TOKEN": "{{ csrf_token() }}",
            },
