...
class StatisticPpkbd extends Controller
{
    /**
    ...
    public function statisticPpkbd()
    {   
        $prequisite['title'] = "Rasio PPKDB";
        return view('statisticppkbd' , $prequisite);
    }

    public function getAllStatPpkbdProv(Request $request)
    {
        ...
        $builder->select(['a.id as province_id' , 'a.name' , 'e.dasar' , 'e.berkembang' , 'e.mandiri' , 'e.jumlahPpkbd']);
		
        $builder->leftjoin('dat_ppkbd_pokkb_jml_prov as e' , 'e.kdprovinsi' , '=' ,'a.id');
        ...
        foreach ($list as $item) {
            ...
            $row[] = number_format($item->dasar, 0, ',', '.');
            $row[] = number_format($item->berkembang, 0, ',', '.');
            $row[] = number_format($item->mandiri, 0, ',', '.');
            $row[] = number_format($item->jumlahPpkbd, 0, ',', '.');
            $row[] = number_format(($jumlahKelurahan / max($jumlahKelurahan, $item->jumlahPpkbd)) * 100, 0, ',', '.') . " : " . number_format(($item->jumlahPpkbd / max($jumlahKelurahan, $item->jumlahPpkbd)) * 100, 0, ',', '.');
    ...
    public function getAllStatPpkbdRegency(Request $request)
    {
        $page = $request->start/$request->length + 1;
        $search = $request->search['value'];
        $province_id = $request->province_id;
        ...
        $builder->select(['a.id as regency_id' , 'a.name' , 'e.dasar' , 'e.berkembang' , 'e.mandiri' , 'e.jumlahPpkbd']);
		
        $builder->leftjoin('dat_ppkbd_pokkb_jml_kokab as e' , 'e.kdkokab' , '=' ,'a.id');
    ...
    public function getAllStatPpkbdDistrict(Request $request)
    {
        $page = $request->start/$request->length + 1;
        $search = $request->search['value'];
        ...
        $builder->select(['a.id as district_id' , 'a.name' , 'e.dasar' , 'e.berkembang' , 'e.mandiri' , 'e.jumlahPpkbd']);
		
        $builder->leftjoin('dat_ppkbd_pokkb_jml_kec as e' , 'e.kdkec' , '=' ,'a.id');
    ...
    function exportPpkbd(Request $request){
        ...
        if(isset($list)){
            ...
            foreach ($list as $item) {
                ...
                $sheet
                    ...
                    ->setCellValue('E' . $column,  number_format($item->dasar, 0, ',', '.'))
                    ->setCellValue('F' . $column, number_format($item->berkembang, 0, ',', '.'))
                    ->setCellValue('G' . $column, number_format($item->mandiri, 0, ',', '.'))
                    ->setCellValue('H' . $column, number_format($item->jumlahPpkbd, 0, ',', '.'))
                    ->setCellValue('I' . $column, number_format(($jumlahKelurahan / max($jumlahKelurahan, $item->jumlahPpkbd)) * 100, 0, ',', '.') . " : " . number_format(($item->jumlahPpkbd / max($jumlahKelurahan, $item->jumlahPpkbd)) * 100, 0, ',', '.'));
            ...
            $sheet
                ...
                ->setCellValue('A2' , "Rasio PPKBD Semua Provinsi")->mergeCells('A2:I2')
                ...
                ->setCellValue('E5' , 'Dasar')
                ->setCellValue('F5' , 'Berkembang')
                ->setCellValue('G5' , 'Mandiri')
            ...
            header('Content-Disposition: attachment;filename="MEKOP RASIO PPKBD PROVINSI  ('. date("d-m-Y").').xlsx"');
    ...
    function regencyExport($request){
        ...
        $builder = DB::table('regencies as a');
        $builder->select(['a.id as regency_id' , 'a.name' , 'c.name as province_name' , 'e.dasar' , 'e.berkembang' , 'e.mandiri' , 'e.jumlahPpkbd']);
        $builder->leftjoin('dat_ppkbd_pokkb_jml_kokab as e' , 'e.kdkokab' , '=' ,'a.id');
        ...
        if(isset($list)){
            ...
            $sheet
                ->setCellValue('A1' , 'DATA KOTA/KABUPATEN PROVINSI '.$province_name)->mergeCells('A1:I1')
                ->setCellValue('A2' , "Rasio PPKBD Semua Kota/Kabupaten Provinsi ".ucwords(strtolower($province_name)," "))->mergeCells('A2:I2')
                ->setCellValue('A4' , '')->mergeCells('A4:I4')
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="MEKOP RASIO PPKBD KOTA_KABUPATEN PROVINSI '.$province_name.' ('. date("d-m-Y").').xlsx"');
            header('Cache-Control: max-age=0');
            $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xlsx');
            return $writer->save('php://output');
        }
    ...
    function districtExport($request){
        $builder = DB::table('dat_ppkbd_pokkb_jml_kec as b');
        ...
        if(isset($list)){
            ...
            foreach ($list as $item) {
                ...
                $sheet
                    ...
                    ->setCellValue('C' . $column, $item->name)
                    ->setCellValue('D' . $column, number_format($item->dasar, 0, ',', '.'))
                    ->setCellValue('E' . $column, number_format($item->berkembang, 0, ',', '.'))
                    ->setCellValue('F' . $column, number_format($item->mandiri, 0, ',', '.'))
                    ->setCellValue('G' . $column, number_format($item->jumlahPpkbd, 0, ',', '.'));
            ...
            $sheet
                ...
                ->setCellValue('A2' , "Rasio PPKBD Kecamatan ".ucwords(strtolower($regency_name)," "))->mergeCells('A2:G2')
                ->setCellValue('A4' , '')->mergeCells('A4:G4')

                ->setCellValue('A5', 'No')
                ->setCellValue('B5', 'Kode')
                ->setCellValue('C5', 'Kecamatan')
                ->setCellValue('D5' , 'Dasar')
                ->setCellValue('E5' , 'Berkembang')
                ->setCellValue('F5' , 'Mandiri')
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="MEKOP RASIO PPKBD KECAMATAN '.$regency_name.' ('. date("d-m-Y").').xlsx"');
            header('Cache-Control: max-age=0');
            $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xlsx');
            return $writer->save('php://output');
        }
