...
class StatisticRealize extends Controller
{
    /**
    ...
    public function statisticRealize()
    {   
        $prequisite['title'] = "Pelaksanaan Mekop";
        return view('statisticrealize' , $prequisite);
    }

    public function getAllStatRealizeProv(Request $request)
    {
        ...
        foreach ($list as $item) {
            ...
            $row[] = number_format($item->districts_count, 0, ',', '.');
            ...
            $row[] = number_format($passed , 0, ',', '.');

            $rasio = 0;
            if($item->districts_count > 0){
                $rasio =  number_format((($item->districts_count / max($passed , $item->districts_count)) * 100), 2, ',', '.') ." : "
                .number_format((($passed / max($passed , $item->districts_count)) * 100), 2, ',', '.')  ;
            }
            $row[] = $rasio;
    ...
    public function getAllStatRealizeRegency(Request $request)
    {
        $page = $request->start/$request->length + 1;
        $search = $request->search['value'];
        $province_id = $request->province_id;
    ...
    public function getAllStatRealizeDistrict(Request $request)
    {
        $page = $request->start/$request->length + 1;
        $search = $request->search['value'];
    ...
    public function getAllStatRealizeVillage(Request $request)
    {
        $page = $request->start/$request->length + 1;
        $search = $request->search['value'];
    ...
    function exportRealize(Request $request){
        ...
        if(isset($list)){
            ...
            foreach ($list as $item) {
                $sheet
                    ...
                    ->setCellValue('D' . $column, number_format($item->districts_count, 0, ',', '.'));
                ...
                $sheet
                    ->setCellValue('E' . $column, number_format($passed, 0, ',', '.'))
                    ->setCellValue('F' . $column, $rasio);
            ...
            $sheet
                ...
                ->setCellValue('A2' , "Rasio Pelaksanaan Mekop Semua Provinsi")->mergeCells('A2:F2')
                ->setCellValue('A4' , '')->mergeCells('A4:F4')

                ->setCellValue('A5', 'No')
                ->setCellValue('B5', 'Kode')
                ->setCellValue('C5', 'Provinsi')
                ->setCellValue('D5', 'Jumlah Kecamatan')
                ->setCellValue('E5' , 'Jumlah Kecamatan yang Melaksanakan Mekop')
                ->setCellValue('F5' , 'Rasio Pelaksanaan')
            ...
            header('Content-Disposition: attachment;filename="MEKOP RASIO PELAKSANAAN MEKOP PROVINSI  ('. date("d-m-Y").').xlsx"');
    ...
    function regencyExport($request){
        ...
        if(isset($list)){
            ...
            $sheet
                ...
                ->setCellValue('A2' , "Rasio Pelaksanaan Mekop Provinsi ".ucwords(strtolower($province_name) , " "))->mergeCells('A2:F2')
                ->setCellValue('A4' , '')->mergeCells('A4:F4')
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="MEKOP RASIO PELAKSANAAN MEKOP KOTA KABUPATEN PROVINSI '.$province_name.'  ('. date("d-m-Y").').xlsx"');
            header('Cache-Control: max-age=0');
            $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xlsx');
            return $writer->save('php://output');
        }
    ...
    function districtExport($request){
        ...
        if(isset($list)){
            ...
            $sheet
                ...
                ->setCellValue('A2' , "Rasio Pelaksanaan Mekop ".ucwords(strtolower($regency_name), " "))->mergeCells('A2:F2')
                ->setCellValue('A4' , '')->mergeCells('A4:F4')
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="MEKOP RASIO PELAKSANAAN MEKOP KECAMATAN '.$regency_name.'  ('. date("d-m-Y").').xlsx"');
            header('Cache-Control: max-age=0');
            $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xlsx');
            return $writer->save('php://output');
        }
    ...
    function villageExport($request){
        ...
        if(isset($list)){
            ...
            $sheet
                ...
                ->setCellValue('A5', 'No')
                ->setCellValue('B5', 'Kode')
                ->setCellValue('C5', 'Desa/Kelurahan')
